...
function getProp(props, key) {
  ...
  if ('registerPaint' in globalThis) {
      ...
      class {
        static get inputProperties() {
          return [
            ...
            '--speechifyElemColor',
            '--speechifySentenceHighlightColor',
          ];
        }
        ...
        paint(ctx, _, props) {
          ...
          // each of the below "positions" variables is an array of numbers, where
          // each group of 4 numbers is a Vec4 representing a rectangle to draw (x,y,w,h)
          ...
          const sentenceColor = getProp(props, '--speechifySentenceHighlightColor');
    ...
    globalThis.registerPaint(
      'speechifySalienceHighlight',
      class {
        static get inputProperties() {
          return [
            '--speechifySalienceHighlightWordInfo',
            '--speechifySalienceHighlightWordClipInfo',
            '--speechifySalienceHighlightSentenceInfo',
            '--speechifySalienceElemColor',
            '--speechifySalienceSentenceHighlightColor',
          ];
        }
  
        paint(ctx, _, props) {
          const elemColor = getProp(props, '--speechifySalienceElemColor');
  
          const wordPositions = getProp(props, '--speechifySalienceHighlightWordInfo')
            ?.split(',')
            .map(Number);
          const wordClipPositions = getProp(props, '--speechifySalienceHighlightWordClipInfo')
            ?.split(',')
            .map(Number);
  
          const sentencePositions = getProp(props, '--speechifySalienceHighlightSentenceInfo')
            .split(',')
            .map(Number);
  
          const sentenceColor = getProp(props, '--speechifySalienceSentenceHighlightColor');
